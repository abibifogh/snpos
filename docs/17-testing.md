...
| `stocktake.test.ts` | Counting the shop's shelves: that blank is not nought, that a line which matched writes nothing, that finding MORE than expected can only be a miscount whatever the dropdown says, that the summary separates a breakage from a disappearance, and that warnings warn without ever refusing. |
