...
| `timing.test.ts` | How long a ticket has, when it is late, whether all its lines arrived, and reading the options chosen on a line — including that unreadable options say so rather than reading as a plain dish. Also that a wait starting before opening time counts from the door, that the hour cap holds the queue back and lets the doors through, and that such a ticket is not late until its promise breaks. |
