...
| `shift-rules.test.ts` | Shift codes per side, the 24 hour limit including a device with the wrong clock, which orders a shift may close over, and that an order once moved is payable on the shift that took it. Also that a voided payment is not money the business holds, and that a row with no status at all is a real one. |
