...
| `timing.test.ts` | How long a ticket has, when it is late, whether all its lines arrived, and reading the options chosen on a line — including that unreadable options say so rather than reading as a plain dish. Also that a wait starting before opening time counts from the door, that a second pre-order queues behind the first one's COOKING rather than its whole quote, that an order with no cooking time recorded never lends its door wait to the queue, that the hour cap sits in front of the customer and not in front of the kitchen, and that such a ticket is not late until its promise breaks. |
| `drink-import.test.ts` | Reading a drinks list from a spreadsheet: that the shipped template survives its own importer, that rows sharing a name become one drink with a recipe, that a missing category is created and said so first, that an ingredient nobody has set up stops the row by name, that a recipe line with no quantity is refused rather than silently ignored, and that the same ingredient twice is a mistake rather than a double measure. |
| `locations.test.ts` | Two warehouses: that a delivery lands in the store and a sale comes off the counter, that one place answers both questions so nothing has to be set up first, that a level is per place and the total is the sum of them, that moving more than the book says is warned about rather than refused, and that a transfer is one act with two ends whose deltas cancel. |
| `bar-count.test.ts` | Counting a bar in and out: that the sheet is grouped in the order a bar is walked rather than alphabetically, that a unit nobody planned for still gets counted, that blank is not nought at the end of a long night, that a repeating remainder is arithmetic rather than a missing drink, that a half-counted sheet cannot close a shift, and that a complete count which is badly short goes to an admin first. |
| `stocktake.test.ts` | Counting the shop's shelves: that blank is not nought, that a line which matched writes nothing, that finding MORE than expected can only be a miscount whatever the dropdown says, that the summary separates a breakage from a disappearance, and that warnings warn without ever refusing. Also grouping the shelf the way somebody walks it with the catch-all group last, that an approved count is applied as a DIFFERENCE so anything sold since still counts, and that one person may sign off their own count visibly. |
| `maker-import.test.ts` | Reading a spreadsheet of makers: that the shipped template survives its own importer, that a code already on file is a correction rather than a duplicate, that two rows claiming one code are caught before either is written, that a rate and a flat amount cannot both apply, and that a blank rate leaves a maker's terms alone. |
| `price-history.test.ts` | What an ingredient has cost over time: that a price reads in the order it moved whatever order it arrives in, that the average is weighted by how much was bought rather than a mean of the prices, that a movement with no price is not a free delivery, and that the note holds its tongue about a wobble. |
| `expense-draft.test.ts` | Keeping a half-typed spend while a cook goes to accept an order: that a draft belongs to one person on one shift, that an untouched form is not unfinished work, that a saved expense takes its draft with it, and that unreadable storage never stops somebody recording money that left the till. |
...
| `shift-rules.test.ts` | Shift codes per side, the 24 hour limit including a device with the wrong clock, which orders a shift may close over, and that an order once moved is payable on the shift that took it. Also that a voided payment is not money the business holds, that a row with no status at all is a real one, and that a till which will not sell says which of the two reasons it is rather than showing an empty box. |
